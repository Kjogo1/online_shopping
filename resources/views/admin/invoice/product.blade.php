...
<form method="POST" action="{{route('admin.invoice.date')}}">
    @csrf
    @method('GET')
    <label for="date" class="text-gray-700 dark:text-gray-400">Pick Up Date</label>
    <input type="date" name="date" onchange="this.form.submit()" @if ($date)
        value="{{$date}}"
    @endif class="rounded-lg">
