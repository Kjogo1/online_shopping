            ...
            <div
                ...
                <div class="flex flex-row items-center justify-between mt-2">
                    ...
                    <form action="{{ route('product.cart', $product->id) }}" method="POST" class="inline">
                        ...
                        @method('GET')
                        <button type="submit"
                            class="text-gray-800 dark:text-white bg-blue-500 rounded-md p-2 uppercase hover:bg-blue-600">
                            Add To Cart
                        </button>
                    </form>
                </div>
                {{-- <div class="flex md:flex-row flex-col justify-between mt-2"> --}}
                    <form method="POST" class="flex md:flex-row flex-col md:justify-between mt-2" action="{{route('product.order')}}">
                        @csrf
                        @method('POST')
                        <div class="flex mt-w-20">
                            <button class="bg-blue-500 rounded-md p-2 hover:bg-blue-600" id="add" type="button">
                            ...
                            <button class="bg-blue-500 rounded-md p-2 hover:bg-blue-600" id="minus" type="button">
        ...
        <div class="mt-5">
            <ul class="flex flex-row scroll-x list-none snap-mandatory w-full">
                @foreach ($related as $product)
                    <li class="mr-5">
                        <span
                            class="flex flex-col w-56 md:hover:scale-105 hover:scale-110 transition delay-150 duration-300 ease-in-out pb-2 rounded-lg shadow bg-white border border-gray-200 hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
                            <img class="w-56 h-56 object-fill rounded-md"
                                src="{{ '/storage/products/' . $product->image }}" alt="Image">
                            <span
                                class="text-gray-700 dark:text-gray-400 text-lg overflow-hidden whitespace-nowrap ml-2">{{ $product->name }}</span>
                            <span class="text-gray-700 dark:text-gray-400 ml-2">Price: ${{ $product->price }}</span>
                            <div class="my-3 ml-2">
                                <a href="{{ route('product.show', $product->id) }}"
                                    class="text-white bg-blue-500 rounded-md p-2 hover:bg-blue-600">See More</a>
                            </div>
                        </span>
                    </li>
                @endforeach
