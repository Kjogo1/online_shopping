        ...
        <div class="flex flex-col gap-4 mb-5 md:w-3/6 ">
            @foreach ($products as $product)
                    <div
                        ...
                        <img class="h-auto md:w-2/6 w-fit rounded-lg object-cover"
                            src="{{'/storage/products/'.$product['item']->image}}" alt="">
                        <div class="mx-2">
                            <div class="absolute animate-bounce inline-flex items-center justify-center md:-right-3 w-6 h-6 text-xs font-bold text-white bg-red-500 border-2 border-white rounded-full -top-2 -right-2 dark:border-gray-900">{{$product['qty']}}</div>
                            ...
                            <div class="flex justify-between flex-col mt-1">
                                <span class="text-gray-800 dark:text-white text-sm md:text-md">Price: ${{$product['item']->price}}</span>
                            </div>
            ...
            <div>
                <form>
                    <button class="bg-blue-500 hover:bg-blue-600 rounded-md p-2 text-white">Checkout</button>
                </form>
            </div>
        </div>
        @else
